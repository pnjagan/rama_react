...
function App(props) {
  ...
  useEffect(() => {
    ...
    if (
      (loginState.data.loggedUser == null ||
        loginState.data.loggedUser == "") &&
      localStorage.getItem("siaUserLogin") != null &&
      localStorage.getItem("siaUserLogin") != ""
    ) {
      dispatch(
        mLoginPost({
  ...
  log(
    ...
    localStorage.getItem("siaJWT") == null ||
      localStorage.getItem("siaUserLogin") == null, //localStorage has no Valid Value
    loginState.meta.status === "FETCH_FAILED" ||
      loginState.meta.status === "NOT_REQUESTED", //To Initiate a new LOGIN
    //Exclusion pages
